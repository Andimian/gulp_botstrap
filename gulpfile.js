...
// таски ==============================================================
task('clean', () => {
    return src("dist/**/*", "!dist/images/icons/map-marker.svg", {read: true}).pipe(rm());
});
...
task ('copy:fonts', () => {
    return src('src/assets/fonts/*.woff2').pipe(dest('dist/fonts')).pipe(reload({stream: true}));
});
task ('copy:video', () => {
    return src('src/assets/video/*.mp4').pipe(dest('dist/video')).pipe(reload({stream: true}));
});
task ('copy:png', () => {
    return src('src/assets/img/**/*.png').pipe(dest('dist/img')).pipe(reload({stream: true}));
});
task ('copy:img', () => {
    return src('src/assets/img/**/*.jpg').pipe(dest('dist/img')).pipe(reload({stream: true}));
});
